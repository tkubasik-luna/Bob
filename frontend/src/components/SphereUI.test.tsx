...
describe("SphereUI — piste shell structure", () => {
  ...
  test("renders the 3D stage with the three depth slots (core has the orb)", () => {
    ...
    // WebGL2 is unavailable under jsdom, so the orb falls back to the
    // `.hud-error` banner — either the canvas or the banner is acceptable proof
    // the orb mounted in the slot.
    const orbMounted =
    ...
    // No label in front of the orb (removed per design).
