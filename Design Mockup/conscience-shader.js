// conscience-shader.js
// WebGL2 fragment shader for Bob's living consciousness.
// Three ORGANIC forms (Souffle / Iris / Murmure) sharing a warm membrane.
//
// Aliveness is driven from JS via uniforms that the life engine integrates:
//   uBreath    organic, slightly-irregular breathing signal (0..1)
//   uGaze      where it is looking, spring-tracked toward cursor (-1..1, xy)
//   uAttention how focused/engaged it is (0..1)
//   uBlink     eyelid / shiver pulse (0..1)
//   uDrift     slow floating offset so it is never perfectly centred (xy)
//   uWobble    asymmetric-deformation amount (0..1)
// Plus per-state weights that crossfade so moods melt into each other.

window.ConscienceShader = (function () {
  ...
  function createRenderer(canvas) {
    ...
    const U = {};
    ['uRes','uTime','uForm','uMotion','uGlow','uAccent','uAccent2','uAccent3','uBg','uAudio',
     'uBreath','uGaze','uAttention','uBlink','uDrift','uWobble',
     'uIdle','uListen','uThink','uSpeak','uAlert','uError',
     'uTrailCount','uTrailSpeed','uTrailLen','uTrailWidth','uTrailAlt','uTrailGlow',
     'uSphereSize','uCoreGlowT','uFogAmt','uIorT','uRimT','uEquator','uLatitude','uOrbitPhase','uTint'].forEach(n => U[n]=gl.getUniformLocation(prog,n));
    ...
    function render(u) {
      ...
      gl.uniform1i(U.uForm, u.form);
      gl.uniform1f(U.uMotion, u.motion);
      gl.uniform1f(U.uGlow, u.glow);
      gl.uniform3fv(U.uAccent, u.accent);
      gl.uniform3fv(U.uAccent2, u.accent2);
      gl.uniform3fv(U.uAccent3, u.accent3);
      gl.uniform3fv(U.uBg, u.bg);
      gl.uniform1f(U.uAudio, u.audio);
      gl.uniform1f(U.uBreath, u.breath);
      gl.uniform2f(U.uGaze, u.gaze[0], u.gaze[1]);
      gl.uniform1f(U.uAttention, u.attention);
      gl.uniform1f(U.uBlink, u.blink);
      gl.uniform2f(U.uDrift, u.drift[0], u.drift[1]);
      gl.uniform1f(U.uWobble, u.wobble);
      gl.uniform1f(U.uIdle, u.states.idle);
      gl.uniform1f(U.uListen, u.states.listen);
      gl.uniform1f(U.uThink, u.states.think);
      gl.uniform1f(U.uSpeak, u.states.speak);
      gl.uniform1f(U.uAlert, u.states.alert);
      gl.uniform1f(U.uError, u.states.error);
      const n = u.neb || {};
      ...
      gl.uniform1f(U.uOrbitPhase, n.orbitPhase || 0.0);
      const tint = u.tint || [1.0, 0.93, 0.86];
