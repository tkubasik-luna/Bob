...
window.ConscienceLife = (function () {
  const STATES = ['idle', 'listen', 'think', 'speak', 'alert', 'error'];

  // Warm / organic palettes per state — mood reads as colour, but all stay in family.
  function hex(h) {
    h = h.replace('#', '');
    return [parseInt(h.slice(0,2),16)/255, parseInt(h.slice(2,4),16)/255, parseInt(h.slice(4,6),16)/255];
  }
  const PALETTES = {
    idle:   { accent: hex('#FF7A45'), accent2: hex('#FFB6A0'), accent3: hex('#FFE4D7'), bg: hex('#0A0605') },
    listen: { accent: hex('#FF9A4E'), accent2: hex('#FFCDA0'), accent3: hex('#FFF0E2'), bg: hex('#0B0706') },
    think:  { accent: hex('#E86A8E'), accent2: hex('#FFB0C6'), accent3: hex('#FFE0EC'), bg: hex('#0B0608') },
    speak:  { accent: hex('#FFA255'), accent2: hex('#FFD8B0'), accent3: hex('#FFF2E2'), bg: hex('#0B0706') },
    alert:  { accent: hex('#FF6A24'), accent2: hex('#FFC24D'), accent3: hex('#FFE6B0'), bg: hex('#0C0604') },
    error:  { accent: hex('#E5443A'), accent2: hex('#FF8A6A'), accent3: hex('#FFD0C0'), bg: hex('#0C0504') },
  };

  // Per-state breathing character: rate (Hz-ish), depth, jitter, and how it holds.
  const BREATH = {
    idle:   { rate: 0.17, depth: 1.00, jitter: 0.10 },
    listen: { rate: 0.13, depth: 0.45, jitter: 0.05 },   // shallow, almost held — attentive
    think:  { rate: 0.26, depth: 0.70, jitter: 0.35 },   // irregular
    speak:  { rate: 0.40, depth: 0.85, jitter: 0.20 },
    alert:  { rate: 0.62, depth: 0.55, jitter: 0.30 },   // fast, shallow
    error:  { rate: 0.50, depth: 0.65, jitter: 0.60 },   // erratic
  };
  ...
  function snoise(x) {
    return 0.5 + 0.34*Math.sin(x*1.0) + 0.12*Math.sin(x*2.37+1.3) + 0.04*Math.sin(x*5.1+2.6);
  }
  ...
  class LifeEngine {
    constructor(opts = {}) {
      ...
      this.weights = { idle:1, listen:0, think:0, speak:0, alert:0, error:0 };

      // colour (melts)
      this.accent  = this.PAL.idle.accent.slice();
      this.accent2 = this.PAL.idle.accent2.slice();
      this.accent3 = this.PAL.idle.accent3.slice();
      this.bg      = this.PAL.idle.bg.slice();

      // breath
      this.breathPhase = 0;     // 0..1 cycle
      this.breath = 0;          // 0..1 output
      this.sighTimer = 4 + Math.random()*5;
      this.sigh = 0;            // extra depth from an occasional deep breath

      // gaze (spring) — in -1..1 look space
      this.gaze = [0, 0];
      this.gazeVel = [0, 0];
      this.gazeTarget = [0, 0];
      this.mouse = [0, 0];      // normalised look target from cursor
      this.mouseActive = 0;     // decays; nonzero = cursor recently moved
      this.saccadeTimer = 1 + Math.random()*2;   // when ignored, look around on its own
      this.wanderTarget = [0, 0];
      ...
      this.blinkPhase = -1;     // -1 idle; else 0..1 progress

      // drift / floating
      this.driftSeedX = Math.random()*100;
      this.driftSeedY = Math.random()*100;
      this.drift = [0, 0];

      this.wobble = 0;

      // simulated audio for speak
      this.audio = 0;

      // exposed vitals for the HUD
      this.vitals = { breath: 0, attention: 0, gaze: [0,0], bpm: 0, mood: 'idle' };
    }

    setState(s) { if (STATES.includes(s)) this.targetState = s; }

    // mousePx: [x,y] in CSS px relative to canvas; size: {w,h}; null = no cursor
    setMouse(mousePx, size) {
      if (!mousePx) { this.mouseActive = Math.max(0, this.mouseActive - 0.02); return; }
    ...
    update(dt, opts) {
      ...
      let blended = { rate:0, depth:0, jitter:0 };
      ...
      if (this.blinkPhase < 0) {
        ...
        if (this.blinkPhase >= 1) {
          this.blinkPhase = -1; this.blink = 0;
      ...
      // ---- voice-like amplitude (speak) ----
      // Fast syllabic bursts + flutter so the body reads as if it's talking.
      const speaking = this.weights.speak;
      const tt = this.t;
      const syllable = Math.pow(0.5 + 0.5*Math.sin(tt*9.0 + Math.sin(tt*2.3)), 2.0); // ~syllable rate
    ...
    uniforms(form, motion, glow) {
      return {
        time: this.t, form, motion, glow,
        accent: this.accent, accent2: this.accent2, accent3: this.accent3, bg: this.bg,
        audio: this.audio,
        breath: this.breath, gaze: this.gaze, attention: this.attention,
        blink: this.blink, drift: this.drift, wobble: this.wobble,
        states: this.weights,
      };
    }
  }

  return { LifeEngine, STATES };
})();
