...
- [0018 LM Studio SDK Inference](docs/features/0017-lmstudio-sdk-inference.md) — LM Studio inference over the `lmstudio` SDK behind `LLM_LMSTUDIO_TRANSPORT` (default `openai`); chat/stream/tool-capture + arg-fragment override; purge (0116) held for real-server validation.
- [0019 Latence orale & fiabilité](docs/features/0018-oral-latency-reliability.md) — Fast/interruptible voice: concurrent endpoint, zero-grace barge-in, pipelined TTS, watchdog + supervised tasks, turn metrics, boot warmup.
